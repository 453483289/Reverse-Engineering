        ...
        bytevec_t &req)
{
  ...
  for ( b=bpts.begin()+nadd; b != bend; ++b )
    verb(("del_bpt(ea=%x, type=%d orgbytes.size=%" FMT_Z " size=%d)\n",
  ...
  for ( b=bpts.begin(); b != bend; ++b )
    verb(("add_bpt(ea=%x type=%d len=%d) => code %d\n", b->ea, b->type, b->size, b->code));
      ...
      case RPC_GET_SREG_BASE:
        {
          ...
          verb(("get_thread_sreg_base(tid=%d, %d) => %x\n", tid, sreg_value, result ? ea : BADADDR));
          append_dd(req, result);
          if ( result )
            append_ea64(req, ea);
        }
      ...
      case RPC_READ_MEMORY:
        {
          ...
          verb(("read_memory(ea=%x size=%" FMT_Z ") => %" FMT_ZS, ea, size, result));
          if ( result > 0 && size == 1 )
            verb((" (0x%02X)\n", *buf));
          else
            verb(("\n"));
      ...
      case RPC_WRITE_MEMORY:
        {
          ...
          verb(("write_memory(ea=%x size=%" FMT_Z ") => %" FMT_ZS, ea, size, result));
          if ( result && size == 1 )
            verb((" (0x%02X)\n", *buf));
          else
            verb(("\n"));
      ...
      case RPC_ISOK_BPT:
        {
          ...
          verb(("isok_bpt(type=%d ea=%x len=%d) => %d\n", type, ea, len, result));
          append_dd(req, result);
        }
        break;
