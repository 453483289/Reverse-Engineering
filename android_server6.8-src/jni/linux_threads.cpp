...
//--------------------------------------------------------------------------
local linux_debmod_t *find_debugger(ps_prochandle *hproc)
{
...
bool linux_debmod_t::tdb_enable_event(td_event_e event, internal_bpt *bp)
{
  ...
  if ( !ok )
  {
    dmsg("%x: failed to add thread_db breakpoint\n", ea_t(notify.u.bptaddr));
    return false;
  }
  debdeb("%x: added BP for thread event %s\n", bp->bpt_addr, event == TD_CREATE ? "TD_CREATE" : "TD_DEATH");
  return true;
}
...
// -1 for unknown,
// 0  if PTRACE_O_TRACEFORK can not be used,
// 1  if it can.
...
static int linux_tracefork_child(void *arg)
{
  errno = 0;
...
#define STACK_SIZE 4096
  clone(linux_tracefork_grandchild, (char *)arg + STACK_SIZE, CLONE_VM | SIGCHLD, NULL);
#else
  qnotused(arg);
  if ( fork() == 0 )
...
#ifdef __UCLINUX__
  char *stack = (char *)qalloc(STACK_SIZE * 4);
  int child_pid = clone(linux_tracefork_child, stack + STACK_SIZE,
...
void linux_debmod_t::handle_extended_wait(bool *handled, const chk_signal_info_t &csi)
{
  ...
  if ( event == PTRACE_EVENT_CLONE )
  {
    ...
    if ( !seen_threads.del(new_pid) )
    {
      ...
      {
        get_thread(new_pid)->waiting_sigstop = true;
        if ( tid2 > 0 )
          store_pending_signal(tid2, status);
      }
    }
