...
char *debug_event_str(const debug_event_t *ev, char *buf, size_t bufsize)
{
  ...
  ptr += qsnprintf(ptr, end-ptr, "%s ea=%x",
  ...
  case LIBRARY_LOAD:   // New library loaded
    ptr += qsnprintf(ptr, end-ptr, " base=%x size=%x rebase=%x name=%s",
                     ev->modinfo.base,
                     ev->modinfo.size,
  ...
  case BREAKPOINT:     // Breakpoint reached
    ptr += qsnprintf(ptr, end-ptr, " hea=%x kea=%x", ev->bpt.hea, ev->bpt.kea);
    break;
  case EXCEPTION:      // Exception
    ptr += qsnprintf(ptr, end-ptr, " code=%x can_cont=%d ea=%x info=%s",
                     ev->exc.code,
                     ev->exc.can_cont,
                     ev->exc.ea,
                     ev->exc.info);
    ...
    if ( code > 0 )
    {
      ...
      if ( code > 0 )
      {
        ...
        if ( dbg_add_bpt(lc->type, lc->ea, bptlen) <= 0 )
        {
          ...
          // any more
          code = 0;
          dwarning("%x: could not restore deleted bpt\n", lc->ea); // odd
        }
      }
      ...
      int ndel)
{
  ...
  for ( b=ubpts; b != end; b++ )
  {
    ...
    if ( code == BPT_OK )
    {
      ...
      debdeb("dbg_add_bpt(type=%d, ea=%x, len=%d) => %d\n", b->type, ea, len, code);
      switch ( code )
      {
        case 2:
          code = BPT_PAGE_OK;
  ...
  for ( ; b != end; b++ )
  {
    ...
    debdeb("dbg_del_bpt(type=%d, ea=%x) => %d\n", b->type, b->ea, code);
    if ( code > 0 )
      cnt++;
    else
...
bool debmod_t::restore_broken_breakpoints(void)
{
  ...
  for ( p = bpts.begin(); p != bpts.end(); ++p )
  {
    ...
    if ( !dbg_write_memory(bpt.ea, bpt.saved, bpt.nsaved) )
      msg("Failed to restore broken breakpoint at 0x%x\n", bpt.ea);
  }
  bpts.clear();
  return true;
}
        ...
        const exception_info_t *ei)
{
  if ( ei == NULL || (ei->flags & EXC_SILENT) == 0 )
  {
    ...
    {
      dmsg("%x: %s -> %x (exc.code %x, tid %d)\n",
           ev->ea, ev->exc.info, ev->exc.ea, ev->exc.code, ev->tid);
    }
    else
    {
      dmsg("%x: %s (exc.code %x, tid %d)\n",
